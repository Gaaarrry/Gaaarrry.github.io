...
// Mr. Scott
// March 18, 2025
// OOP Reveiw + Object-Object Interactions

//Global Variables
let nodes = []; //to hold all our objects
let reach  = 150; //how far connections b/w points maybe
function setup() {
...
function draw() {
  background(220);
  //if (mouseIsPressed)nodes.push(new csNodes(mouseX, mouseY));
  for(let n of nodes){
...
function mousePressed(){
  //create one node per mousepressed 
  nodes.push(new csNodes(mouseX, mouseY));
}
class csNodes{
  //1. constructor
  constructor(x,y){//called once per object created
    //properties related to position/redering
  ...
  connect(nodeArray){
    ...
    //other points, and of so join with a line
    stroke(this.c);
    for(let n of nodes){
